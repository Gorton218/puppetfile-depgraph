...
export class VersionParser {
    ...
    if (constraint.includes('x')) {
      ...
      if (nonWildcardParts.length > 0) {
        ...
        if (nonWildcardParts.length === 1) {
          const major = Number.parseInt(nonWildcardParts[0]);
          requirements.push({ operator: '<', version: `${major + 1}.0.0` });
        } else if (nonWildcardParts.length === 2) {
          const major = Number.parseInt(nonWildcardParts[0]);
          const minor = Number.parseInt(nonWildcardParts[1]);
  ...
  private static compareVersions(v1: string, v2: string): number {
    ...
    const major1 = Number.parseInt(parse1[1]);
    const major2 = Number.parseInt(parse2[1]);
    if (major1 !== major2) {return major1 - major2;}
    
    const minor1 = Number.parseInt(parse1[2]);
    const minor2 = Number.parseInt(parse2[2]);
    if (minor1 !== minor2) {return minor1 - minor2;}
    
    const patch1 = Number.parseInt(parse1[3]);
    const patch2 = Number.parseInt(parse2[3]);
